...
应用右上角有 **「测试响铃」** 按钮：点一下 → 立刻把应用从后台划掉 → 1 分钟后应该响。
用这个能在 1 分钟内验证全部权限是否到位，不用等到晚上 9 点。

| 现象 | 原因 | 解决 |
| --- | --- | --- |
| 有声音，但通知栏没有「关闭提醒」 | **通知权限被拒**（通知不显示，按钮也跟着没了） | 应用顶部红色提示条 → 「去开启」；或 设置 → 应用 → 备忘提醒 → 通知 → 允许 |
| 把应用划掉后到点不响 | 系统清掉了应用的后台闹钟（国产 ROM 常见） | ① 打开**自启动**、**后台运行**、**锁屏显示**；② 电池 → 省电策略选**无限制**；③ 最近任务里给应用**加锁**；④ 开启「闹钟和提醒」权限 |
| 响了但锁屏看不到界面 | 全屏通知被禁（Android 14+） | 应用顶部提示条 → 「去开启」，或设置里允许「全屏通知」 |
| 时间不准、晚几分钟 | 没有精确闹钟权限 | 应用顶部提示条 → 开启「闹钟和提醒」 |

响铃时的可靠性设计：

- 提醒时刻用系统最强的 `setAlarmClock()` 排布（不受省电模式 / Doze 影响，状态栏会显示闹钟图标）
- 响铃由前台服务循环播放系统默认闹钟音，直到手动关闭
- **通知栏**和**应用界面**都有关闭按钮；锁屏时会直接弹出应用界面（全屏通知）
- 兜底：进程被杀后，每 3 分钟一次的循环闹钟会重新拉起响铃
